...
import AnimeDetailModal from '../../../../common/AnimeDetailModal';
...
const HomeSlider = () => {
    ...
    const { setAllInfo} = useAnimeCardStore();
    ...
    const showModal = async (id) => {
        const data = await fetchDetailedAnimeInfo(id); // 데이터 가져오기
        setShowDetailCard(!showDetailCard); // 모달 표시
        setAllInfo(data);
    };

    // 애니메이션 상세 정보 가져오기
    const fetchDetailedAnimeInfo = async (id) => {
    ...
    <div>
        ...
        >   
            {animationList.map((item, index)=>(
                <SwiperSlide key={index} onClick={() => showModal(item.id)}>
